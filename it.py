...
def run_request(taskQueue, resultQueue, marketplace, goalTime, s1, s2, asin_count, total_asin_count):
    ...
    db_time = ''
    chunk_count = 0
    filename = '_it/' + str(s1) + f"_it_{random.randint(100, 999)}.csv"
    with io.open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            if response == 'error':
                ...
                if tokens < 1000:
                    ...
                    telegram_bot_send('-558961676',
                                      f"****   Token Waning - Amazon IT   ***** +%0A+Token Left :  {tokens}")
                    time.sleep(1000)
        ...
        telegram_bot_send('-552890666',
                          f"IT -- 100 ASIN: {chunk_count}+%0A+ConsumedToken: {response['tokensConsumed']}+%0A+RemainToken: {response['tokensLeft']}+%0A+KeepaResponseTime: {response_time}+%0A+ParsingTime: {parsing_time}+%0A+TotalTIme:  {str(datetime.now() - chunkstart)}")
...
def main():
    ...
    for f in os.listdir('./_it/'):
        if not f.endswith(".csv"):
            continue
        os.remove(os.path.join('_it/', f))
    telegram_bot_send('-558961676',
                      f"IT Report+%0A+Total ASIN: {total_asin_count}+%0A+ConsumedToken: {consumed_token}+%0A+TotalTIme:  {str(datetime.now() - startTime)}")
